...
## 7. WEB APP DECISIONS (Phase 3)

---

### DECISION: Next.js 14 App Router Over React Native Web or Plain HTML

**ALTERNATIVES CONSIDERED**:

- React Native Web (Expo web) — shares codebase with mobile app; but Expo web support is less mature, camera APIs don't work in browser, HEIC conversion requires polyfills
- Plain HTML + vanilla JS — lowest complexity but no SSR, no API routes, no auth session management built-in
- Next.js Pages Router — still supported but App Router is the modern standard with server components, better for Supabase SSR integration

**RATIONALE**:
Next.js 14 App Router was chosen because: (1) **Supabase SSR via `@supabase/ssr`** — official package handles cookie-based auth for server components; mobile uses client-side auth only. (2) **API Routes** — `/api/*` endpoints for MiniMax AI calls and CLIP embeddings without exposing keys to client. (3) **Server Components** — closet page can fetch wardrobe items server-side, reducing client bundle. (4) **Vercel deployment** — zero-config with auto-deploy on push to main. (5) **Existing React/TypeScript familiarity** from mobile codebase.

**TRADE-OFFS ACCEPTED**:

- Next.js has more complexity than vanilla HTML — App Router conventions (server components, dynamic routes, middleware) require understanding
- `@supabase/ssr` requires `export const dynamic = "force-dynamic"` on all Supabase-dependent pages to prevent SSG build failures
...
- Tailwind CSS v3 with `tailwind.config.ts` — v3 uses JS config file, v4 uses CSS `@theme` block
- CSS Modules with custom properties — more control but slower development, no utility-first
- Styled-components — popular but adds runtime overhead, SSR complexity

**RATIONALE**:
Tailwind v4 was chosen because: (1) **CSS-first config** — custom design tokens defined in `globals.css` `@theme` block rather than `tailwind.config.ts`. (2) **Mobile-first responsive utilities** built-in. (3) **Dark mode** support if needed later. (4) ** JIT compilation** — only generates CSS for used classes. The v4 CSS-first approach aligns with Next.js 14's server component model where CSS is already cascade-based.

**TRADE-OFFS ACCEPTED**:

- Tailwind v4 is newer (released 2024) — fewer community resources; most tutorials reference v3 syntax
- `tailwind.config.ts` with `theme.extend` is ignored in v4 — all tokens must be in `@theme` block

---

### DECISION: MiniMax for Web Avatar Generation (Not OpenAI DALL-E or Stable Diffusion)

**ALTERNATIVES CONSIDERED**:

- OpenAI DALL-E 3 — higher quality but more expensive, requires OpenAI account, different style (photorealistic vs fashion illustration)
- Stable Diffusion via Replicate — requires LoRA for fashion illustration style, more setup
- No avatar (mobile only) — reduces feature parity between platforms

**RATIONALE**:
MiniMax Image API was chosen because: (1) **Same provider as mobile** — consistency across platforms; user only needs one API key. (2) **Fashion illustration style** — MiniMax's output is stylized/illustrated, appropriate for "digital twin" rather than photorealistic. (3) **Cost efficiency** — free tier available, consumption-based pricing. (4) **Speed** — 3-8 second generation typical. (5) **Avatar generation** via `txt2img` endpoint with fashion-focused prompt.

**TRADE-OFFS ACCEPTED**:

- MiniMax key is separate from Supabase/HuggingFace — users need to manage multiple API keys
- MiniMax not configured by default — web app handles gracefully with friendly message

---

### DECISION: Separate Supabase Storage Buckets (wardrobe-items vs avatars)

**ALTERNATIVES CONSIDERED**:

- Single bucket for all images — simpler but harder to manage RLS policies
- External CDN (Cloudinary) — more features but another vendor/service

**RATIONALE**:
Two separate buckets: `wardrobe-items` for clothing photos and `avatars` for profile/twin photos. This allows: (1) **Separate RLS policies** — wardrobe items are private to user, avatars may be shareable. (2) **Separate cleanup** — deleting an item removes only clothing photos. (3) **Path structure** — `{userId}/{uuid}.jpg` for wardrobe items, `{userId}/avatar.jpg` for profile photos.

**TRADE-OFFS ACCEPTED**:

- Two buckets = two sets of RLS policies to maintain
- Bucket names must match between mobile and web for consistency

---

### DECISION: Route Middleware for Auth Protection (/app/\* → /auth)

**ALTERNATIVES CONSIDERED**:

- Client-side auth checks (useEffect + redirect) — causes flash of protected content before redirect
- Server-side checks in each page — code duplication
- Third-party auth middleware — adds dependency

**RATIONALE**:
Next.js `middleware.ts` at root intercepts all requests: checks Supabase session cookie, redirects to `/auth` if no session and trying to access `/app/*`. This is: (1) **Server-side** — runs before page renders, no flash. (2) **Centralized** — one file handles all protected routes. (3) **Official Supabase pattern** — `@supabase/ssr` provides the middleware helper.

**TRADE-OFFS ACCEPTED**:

- Middleware runs on every request — adds latency for static assets (mitigated by matcher excluding `_next/static`)
- Auth callback (`/auth/callback`) must be excluded from protection

---

### DECISION: `<img>` Tags Instead of Next.js `<Image>` for User Photos

**ALTERNATIVES CONSIDERED**:

- Next.js `<Image>` component — optimized images, lazy loading, automatic WebP conversion, but requires `remotePatterns` config for each domain
- External image CDN (Supabase built-in) — auto-serves optimized images but still requires domain allowlist

**RATIONALE**:
User-uploaded photos from Supabase Storage and external seed data sources (picsum.photos, unsplash) may use domains not pre-configured in `next.config.ts`. Using `<img>` with `object-fit: cover` avoids hostname errors while still looking premium. Next.js `<Image>` is reserved for internally-controlled assets where domain can be pre-configured.

**TRADE-OFFS ACCEPTED**:

- No automatic image optimization for user photos — acceptable for demo-scale apps
- `<img>` doesn't have lazy loading by default — CSS `loading="lazy"` attribute added manually
- No automatic WebP conversion — larger file sizes possible

---

### DECISION: Ivy + Rose Gold Design System (Not Dark Mode or Other Palettes)

**ALTERNATIVES CONSIDERED**:

- Dark mode — modern but fashion apps typically benefit from seeing true colors of clothing
- Neutral grayscale + single accent — too corporate, doesn't convey "fashion-forward"
- Bright/pastel palette — less premium feeling

**RATIONALE**:
Design system: ivory (#F5F0EA) background, rose gold (#C9847A) primary, charcoal (#2B2B2B) text. This was chosen because: (1) **Fashion-forward premium** — rose gold connotes luxury without being garish. (2) **Ivory background** — shows clothing colors accurately (vs white which can blow out bright colors in photos). (3) **Charcoal text** — softer than pure black, more editorial feel. (4) **Female-skewing demographic** — rose gold appeals to target 22-35 urban Indian women.

**TRADE-OFFS ACCEPTED**:

- Rose gold may not appeal to all users — future: theme picker (Phase 2)
- Ivory shows dirt/fabric texture more than white — acceptable tradeoff for color accuracy
