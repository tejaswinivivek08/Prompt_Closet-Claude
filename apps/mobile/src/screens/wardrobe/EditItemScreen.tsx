...
export default function EditItemScreen({
  ...
  useEffect(() => {
    async function fetchItem() {
      ...
      const aiTags: ClothingTags = data.ai_tags ?? {};
      setCategory(data.category ?? aiTags.category ?? "top");
      setSubcategory(data.subcategory ?? aiTags.subcategory ?? "");
      setColors(data.colors ?? aiTags.colors ?? []);
      setPattern(data.pattern ?? aiTags.pattern ?? "solid");
      setOccasions(data.occasions ?? aiTags.occasions ?? []);
      setFormalityScore(data.formality_score ?? aiTags.formality_score ?? 3);
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      const aiTags: ClothingTags = item?.ai_tags ?? {};
      const updatedAiTags: ClothingTags = {
        ...aiTags,
        category,
        subcategory,
        colors,
        pattern,
        occasions,
        formality_score: formalityScore as 1 | 2 | 3 | 4 | 5,
        season: aiTags.season ?? ["all-season"],
      };
